...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        ### Compute log probability distribution over the target vocabulary using the combined_outputs returned by the `self.decode()` function.
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X=self.model_embeddings.source(source_padded) #(src_len, b)->(src_len, batch_size, e)
        X=nn.utils.rnn.pack_padded_sequence(X,source_lengths)##(src_len, b, e)
        enc_hiddens,(last_hidden,last_cell)=self.encoder(X)## last_hidden : (num_layers * num_directions, batch, hidden_size)=(2,batch_size,hidden_size)
        enc_hiddens,_=nn.utils.rnn.pad_packed_sequence(enc_hiddens)#(src_len, batch_size, hidden_size * 2)
        enc_hiddens = enc_hiddens.transpose(0, 1)# #(batch_size, src_len, hidden_size * 2)
        
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), 1)# last_hidden:(batch_size,hidden_size*2)
        init_decoder_hidden = self.h_projection(last_hidden) #init_decoder_hidden:(batch_size,hidden_size)
        
        last_cell   = torch.cat((last_cell[0], last_cell[1]), 1)#last_cell:(batch_size,hidden_size*2)
        init_decoder_cell   = self.c_projection(last_cell)#init_decoder_cell:(batch_size,hidden_size)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #enc_hiddens: (b, l, h * 2) dot (h*2,h) -> enc_hiddens_proj(b,l,h)
        enc_hiddens_proj=self.att_projection(enc_hiddens)
        
        # Y:(tgt_len,b,embed_size)
        Y = self.model_embeddings.target(target_padded)

        for Y_t in torch.split(Y,1,dim=0):#Y_t:(1,b,embed_size)
            squeezed = torch.squeeze(Y_t) # squeezed:(b, embed_size)
            Ybar_t = torch.cat((squeezed, o_prev), dim=1) # shape (b, e + h)
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)#dec_state:(b,h) o_t:(b,h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(alpha_t.view(*att_view), enc_hiddens).squeeze(1) #a_t:(b, 2h)
