...
def naiveSoftmaxLossAndGradient(
    ...
    ### YOUR CODE HERE
    scores = np.matmul(outsideVectors, centerWordVec)  # (vocab_size*embdeding_size)*(embedding_size*1)=(vocab_size,1)
    probs = softmax(scores)  # (vocab_size,1)  y_hat
    loss = -np.log(probs[outsideWordIdx])

    dscores = probs.copy()  # (vocab_size,1)
    dscores[outsideWordIdx] = dscores[outsideWordIdx] - 1  # y_hat - y
    gradCenterVec = np.matmul(outsideVectors.T, dscores)  # U^T(\hat y-y) -> (vocab_size*embdeding_size)^T*(vocab_size*1)=(embedding_dim,1)
    gradOutsideVecs = np.outer(dscores, centerWordVec)  # (\hat y-y)v_c -> outer((vocab_szie,1)(embedding_dim*1))=(vocab_size,embedding_dim)
